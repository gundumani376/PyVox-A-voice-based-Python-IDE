...
class VoicePythonIDE:
    ...
    def setup_gemini_api(self):
        """Setup Gemini API - Replace 'your-api-key-here' with actual API key"""
        try:
            # You need to set your Gemini API key here
            api_key = "AIzaSyBwepEkfaQqeTrKKhJuERytq-S2SLLl2Uk"  # Replace with your actual API key
    ...
    def setup_gui(self):
        ...
        # Right panel - Chat and voice controls
    ...
    def setup_toolbar(self, parent):
        """Setup toolbar with file operations and voice controls"""
    ...
    def setup_left_panel(self, parent):
        """Setup left panel with code editor and terminal"""
    ...
    def setup_right_panel(self, parent):
        """Setup right panel with chat and voice controls"""
        # Chat section
    ...
    def setup_voice_recognition(self):
        """Setup voice recognition with microphone calibration"""
        try:
    ...
    def append_to_terminal(self, text):
        """Append text to terminal (thread-safe)"""
        def _append():
    ...
    def handle_voice_command(self, command):
        ...
        elif any(phrase in command for phrase in ["write a program", "write code", "create a program", "generate code"]):
    ...
    def handle_code_generation_request(self, command):
        """Handle code generation requests by generating code and inserting it into the editor."""
        self.add_chat_message("You", command)
        
        if self.gemini_available:
            try:
                # Enhanced prompt for code generation
                # This prompt specifically asks the model to ONLY return Python code,
                # without markdown formatting or conversational text.
                prompt = f"""You are a Python code generator. The user said: "{command}"
                
Please generate clean, well-commented Python code based on this request. 
Only return the Python code itself, without any markdown formatting (like ```python) or conversational explanations.
...
"""
                ...
                # Attempt to clean up any unwanted markdown or leading/trailing text.
                # This regex looks for code blocks, but the prompt should ideally prevent them.
                code = raw_code_output
                
                # Check for standard markdown code blocks and extract content
                match = re.search(r"```(?:python)?\s*(.*?)(?:```|$)", raw_code_output, re.DOTALL)
                if match:
                    code = match.group(1).strip()
                else:
                    # If no markdown block is found, assume the entire output is code,
                    # and remove common leading/trailing conversational phrases.
                    # This is a fallback if the prompt isn't perfectly followed.
                    common_prefixes = ["here is the code:", "here's the code:", "python code:"]
                    for prefix in common_prefixes:
                        if code.lower().startswith(prefix):
                            code = code[len(prefix):].strip()
                    
                # Insert the cleaned code into the code editor
                self.code_editor.delete('1.0', tk.END)
            ...
            # Fallback to template generation if Gemini API is not available
            self.add_chat_message("Assistant", "Gemini API is not configured. Falling back to template generation.")
            self.generate_template_code(command)

    
    def generate_template_code(self, command):
...
read_file()'''
        ...
        if not code:
            code = '''# Generated Python Template
# Add your code here
...
if __name__ == "__main__":
    main()'''
        ...
        self.code_editor.delete('1.0', tk.END)
        self.code_editor.insert('1.0', code)
        self.add_chat_message("Assistant", "I've added a code template to the editor. Gemini API is not configured for advanced code generation.")
    
    def process_chat_command(self, command):
        """Process chat commands, including action-oriented ones and general queries"""
        command = command.lower().strip() # Normalize command for consistent handling
        self.add_chat_message("You", command)
        
        # Check for specific action-oriented commands first (same logic as handle_voice_command)
        if "run the file" in command or "execute the code" in command:
            self.run_code()
            self.add_chat_message("System", "Running the code...")
            ...
            # If no specific action command, then proceed with AI chat or fallback responses
            if self.gemini_available:
                try:
                    prompt = f"""You are a helpful Python programming assistant. The user said: "{command}"
                    
Provide a helpful response. If it's about programming, give practical advice.
...
if __name__ == "__main__":
    ...
    # pip install tkinter speechrecognition pyttsx3 google-generativeai pyaudio
    
    print("Starting Voice-Activated Python IDE...")
    print("Make sure you have installed the required packages:")
    print("pip install speechrecognition pyttsx3 google-generativeai pyaudio")
    print("\nDon't forget to set your Gemini API key in the code!")
